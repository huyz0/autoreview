//! Conformance test for every `kind: threshold` rule, parallel to
//! `rule_pack_conformance.rs`/`taint_rule_conformance.rs` — same fixture
//! convention, driven through `autoreview_core::run_complexity_check`
//! (the real end-to-end path: resolve YAML thresholds, scan the file)
//! since a `kind: threshold` rule has no `rule:` block either.

use std::path::{Path, PathBuf};

use autoreview_core::run_complexity_check;
...
fn discover_threshold_rules() -> Vec<RuleMeta> {
    let mut rules = Vec::new();
    walk(&rules_dir(), &mut rules);
    rules.retain(|r| r.kind == "threshold");
...
#[test]
fn every_threshold_rule_fires_on_its_positive_fixture_and_stays_silent_on_its_negative_fixture() {
    let rules = discover_threshold_rules();
    assert!(!rules.is_empty(), "expected to discover at least one kind: threshold rule under {}", rules_dir().display());
    ...
    for rule in &rules {
        ...
        let positive_findings = run_complexity_check(positive_dir.path(), &[filename.to_string()]);
        ...
        let negative_findings = run_complexity_check(negative_dir.path(), &[filename.to_string()]);
    ...
    assert!(failures.is_empty(), "threshold rule conformance failures ({} rule(s) checked):\n{}", rules.len(), failures.join("\n"));
}

#[test]
fn every_threshold_rule_declares_a_category() {
    for rule in discover_threshold_rules() {
        assert!(!rule.category.is_empty(), "{}: kind: threshold rule has no category field", rule.id);
    }
}

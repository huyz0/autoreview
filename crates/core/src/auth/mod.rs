//! Credential storage and login flows for the two external hosts this
//! project can talk to directly: GitHub (`github_device_flow`, RFC 8628
//! OAuth device flow) and Bitbucket Cloud (`bitbucket_token`, HTTP Basic
//! auth with an Atlassian API token). `credential_store` is the shared
//! piece both build on — see its own module doc for the storage-order
